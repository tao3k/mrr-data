...
# Later, after reconnecting: resource.sync_pending() or resource.sync(root=protected.root, ...)
...
then fsyncs a =snapshot-<root>.json= outbox record with source, revision,
generation, closure, state and protection deadline. =pending_roots()= discovers
these roots after restart. =sync_pending()= replays a bounded batch without the
caller reconstructing each scope; =--serve= repeats this under Tokio. =sync=
publishes children before the root and marks the record synced only after S3
acknowledges the root. Repeating =sync= is safe for immutable blocks. Pending
closures remain protected even past their deadline until acknowledged. Expired,
synced records and unshared blocks are removed on the next protect or replay.
The default protection lease is seven days; =MRR_PROTECTION_SECS= changes it.
=MRR_LOCAL_MAX_BYTES= (default 1 GiB) bounds persisted CID blocks for both
protection and S3 read-through fills. A verified remote query still succeeds
when its disposable cache fill cannot fit; a repeat query then reads S3 again.
This bound does not count temporary staging files, record metadata, or writes
by processes outside this worker.
=MRR_SYNC_BATCH= (default 16, maximum 128) limits each replay cycle, and
=MRR_SYNC_INTERVAL_SECS= (default 60) sets the =--serve= retry interval. The
service requires the same S3 configuration as manual publication. A one-shot
replay reports attempted, published and failed roots; failed roots remain pending.
...
local stores must issue zero remote operations. A capacity-limited local store
must still admit the remote result while refusing persistent cache fills, so
its repeat query reads S3 again. Each operation is already a
separate process. This proves clean process restart and outbox discovery;
crash injection during record creation remains a separate qualification gate.
