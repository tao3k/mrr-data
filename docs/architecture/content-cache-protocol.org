...
It does not run capacity eviction or background GC. Kache is an optional
acceleration tier and may evict under its configured policy; an eviction-capable
cache cannot establish a minimum offline-retention promise. A future bounded
retention policy must protect an admitted snapshot root and its complete child
closure for the whole lease, then hand only expired closures to a cache engine
or explicit collector.
