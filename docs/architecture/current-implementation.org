...
The public ~mrr-data~ facade now defaults to ~arrow,filesystem~. Its dependency-light
~mrr-data-profile~ crate owns namespaces and numeric versions without importing
CID, DAG-CBOR, CAR, or GraphAr native dependencies. Verified local filesystem
storage is part of the default graph; CAR packaging remains opt-in.
...
deadline. The original Healthcare GQL worker compiles through MRR's frontend,
restores the catalog-bound property snapshot, executes this physical path, and
submits the candidate to MRR admission. The property-row producer materializes
deterministic Arrow/CID blocks with source and encoded-byte budgets; its test
publishes and restores those blocks before executing the original source. This
library path passed Ubuntu and macOS CI in [[file:healthcare-query-audit.org][the Healthcare query audit]].
The actual Lambda/Marlin process has not yet transferred its POO rows through
a typed native boundary or invoked this worker; that remains a separate gate.
...
With the explicit ~content-identity,graphar~ feature composition, downstream code can
...
The bounded POO Flow static-edge worker owns a durable local outbox above the
default filesystem store. A protection receipt follows complete closure
verification and a durable pending record write; the record binds source, revision and
generation. Tokio's one-shot and continuous replay paths publish children
before the root, then persist remote acknowledgement. Pending closures survive
lease expiry, while expired acknowledged closures are collected on subsequent
activity. Protection and S3 read-through fills share a persisted-CID byte
bound. A remote query can still return verified bytes when a disposable fill
does not fit; a later query then fetches from S3 again.
...
are in [[file:0001-mrr-data-plane.org][RFC 0001]]. Its snapshot distribution delivery plan
...
[[file:../../tools/s3-conformance/README.org][local S3 runner]] needs no cloud account.
The [[file:s3-kache-transport-audit.org::*Final remote acceptance][transport acceptance audit]]
...
cargo check -p mrr-data --no-default-features --features content-identity,graphar --locked
cargo check -p mrr-data --no-default-features --features datafusion --locked
cargo test -p mrr-data --no-default-features --locked
cargo test -p mrr-data --no-default-features --features content-identity,graphar --locked
#+end_src
...
| ~content-identity~ | yes, through ~filesystem~ | CID/DAG-CBOR identity and immutable manifests |
| ~content~ | yes, through ~filesystem~ | verified blocks, memory store and protocol; implies ~content-identity~ |
...
| ~filesystem~ | yes | local filesystem store; implies ~content~ |
| ~cache~ | no | content protocol and Kache local cache |
| ~s3~ | no | content protocol and S3 remote adapter |

~default = ["arrow", "filesystem"]~ provides typed interchange and durable local
storage without a network dependency. ~cache~ and ~s3~ imply ~content~
...
Member crates also default to no optional providers: ~mrr-data-core/content-identity~,
...
The optional [[file:../../integrations/poo_flow/README.org][POO Flow resource]] publishes
