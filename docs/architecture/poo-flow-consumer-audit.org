#+title: POO Flow static-edge consumer acceptance — 2026-09-20
...
kill-during-publication recovery remains qualified by the transport tests.
...
consumer receipt inside the S3 artifact. The transport aggregate therefore includes these
...
jobs, Rust 1.95 MSRV, Arrow IPC fuzz and the transport aggregate. Reviewed final-run logs
...
[[https://github.com/tao3k/mrr-data/pull/1][PR #1]] recorded the static-edge delivery;
that earlier acceptance did not imply Healthcare query or release acceptance.

The [[file:0001-mrr-data-plane.org][canonical roadmap]] separates original Healthcare
query delivery from reuse and real-workload qualification.
