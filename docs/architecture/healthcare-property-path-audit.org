#+title: Healthcare property-path chain audit — initial baseline

This is the initial pre-implementation audit. The original GQL worker and
Arrow/CID property snapshot producer have since been delivered and accepted on
Ubuntu and macOS; [[file:healthcare-query-audit.org][the current Healthcare query audit]] records that evidence.
The real Lambda/Marlin typed native handoff remains open.
...
This audit starts from the squash-merged [[https://github.com/tao3k/mrr-data/pull/1][mrr-data PR #1]]
at ~f53909884efcb9b345837c1fd2746d2cdcdd0c5b~. Its final
[[https://github.com/tao3k/mrr-data/actions/runs/35804588191][CI run]] passed all
nine jobs, including both platforms, the MSRV, GraphAr, S3/TLS, IPC fuzz and snapshot distribution
acceptance. That run qualifies the existing physical slice and static-edge
consumer; it is not an end-to-end Healthcare query receipt.
...
These are dependencies of one Healthcare query acceptance claim, with one owner
per contract.
...
cannot establish either. The ~profile-impact.gql~ and causal-event queries
follow the admitted case-profile path and need their own domain semantics.
